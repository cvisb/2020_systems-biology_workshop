...
1) First, we need to import the data using the function 'read_excel'. 
...
Response variable:
The phenotype we want to predict is latent Mtb infection (LTBI) or resistors (RSTR). The classification is based on the diagnosis. 
Specifically, LTBI and RSTR are defined in paper as following:
* LTBI: A population is diagnosed based on evidence of immune sensitization to Mtb (Mycobacterium tuberculosis) antigens and the absence of clinical symptoms of tuberculosis (TB) or direct microbiologic evidence of disease.
* RSTR: a population of house hold contacts which were highly exposed to Mtb yet remained persistently IGRA (interferon-r (IFN-r) release assay) and TST (tuberculin skin test) negative over an average of 9.5 years of follow-up for each individual. 
...
In the following, we will visualize the IgG1 values for the individual classes across antigens. We annotate the significance using the Wilcox test with the library 'ggpubr'. Note: here, the p-values are not yet adjusted for multiple testing correction!
...
indFeatures <- which(grepl("IgG1.1000", colnames(X))) 

# Reformat the data for the following plotting procedure
dfBox <- reshape2::melt(cbind(as.data.frame(X[,indFeatures]), class = y), id.var = "class")
